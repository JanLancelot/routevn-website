...
export function validateCatalog(data) {
  ...
  for (const novel of data.novels) {
    ...
    for (const key of ['gameUrl', 'vndbUrl']) {
      ...
      let url;
      try {
        url = new URL(novel[key]);
      } catch {
        throw new Error(`${novel.slug}: ${key} must be a valid HTTP or HTTPS link.`);
      }
      if (!['http:', 'https:'].includes(url.protocol)) throw new Error(`${novel.slug}: ${key} must be an HTTP or HTTPS link.`);
    }
...
export function prepare() {
  ...
  mkdirSync(path.join(root, 'pages/en/visual-novel-directory/v'), { recursive: true });
  for (const file of readdirSync(path.join(root, 'pages/en/visual-novel-directory/v'))) {
    if (file.endsWith('.yaml') && !expectedPages.has(file)) rmSync(path.join(root, 'pages/en/visual-novel-directory/v', file));
  }
  for (const item of data.novels) {
    ...
    const host = new URL(item.gameUrl).hostname.replace(/^www\./, '');
    const gameHost = host.endsWith('.itch.io') ? 'itch.io' : host;
    const ogImage = isHttpsUrl(item.cover.src) ? item.cover.src : `https://routevn.com${item.cover.src}`;
    const frontmatter = yaml.dump({ template: 'directory', url: `/en/visual-novel-directory/v/${item.slug}/`, seoTitle: `${item.titleOriginal || item.title} · ${data.site.name}`, seoDescription: item.summary, ogImage, item: { ...item, gameHost } }, { lineWidth: -1 });
    write(`pages/en/visual-novel-directory/v/${item.slug}.yaml`, `---\n${frontmatter}---\n- $partial: directory/novel-detail\n  item: \${item}\n`);
  }
  ...
  write('static/public/directory/catalog-data.js', `export default ${JSON.stringify({ novels, filters: data.filters, copy: { searchLabel, resultOne, resultMany } }, null, 2).replace(/</g, '\\u003c')};\n`);
