...
if (back) back.setAttribute('href', `/en/visual-novel-directory/${queryString(readFilters(location.search, filters), filters)}`);

if (surface) {
  const search = document.querySelector('#search');
  const controls = Object.fromEntries(['language', 'level', 'genre'].map((key) => [key, document.querySelector(`#${key}`)]).filter(([, control]) => control));
  ...
  function refreshLevelOptions() {
    if (!controls.level || activeLanguage === state.language) return;
  ...
  function render() {
    ...
    for (const card of cards) {
      ...
      card.querySelector('a').href = `/en/visual-novel-directory/v/${card.dataset.slug}/${query}`;
    }
  ...
  function update(key, value) {
    ...
    } catch { /* Safari rate-limits replaceState; the URL may lag behind the filters. */ }
    render();
  }
  ...
  restore();
  document.documentElement.classList.add('dir-live');
}
