...
export function readFilters(search, filters) {
  ...
  return {
    q: (params.get('q') ?? '').trim(),
    language,
    level: filters.level.enabled !== false && levelOptions(filters, language).some((option) => option.value === level) ? level : '',
...
export function filterNovels(novels, state, filters) {
  ...
  return novels.filter((novel) => {
    ...
    if (filters.level.enabled !== false && state.level) {
...
export function queryString(state, filters) {
  ...
  for (const key of ['q', 'language', 'level', 'genre']) {
    if (key === 'level' && filters.level.enabled === false) continue;
